---
title: "AtlasVPM - Análisis variaciones en cirugías frecuentes"
subtitle: "Procedimientos incluídos en decreto de garantía de tiempos de espera"
lang: es
author:
...
plot_evo_temp_ccaa <- function(ind){
    ...
    rate_dsr <- rate_dsr %>% filter(!(d_regires_new %in% 'Galicia' & año %in% 2020))
    rate_dsr <- rate_dsr %>% filter(!(d_regires_new %in% 'Castilla y León' & año %in% 2020))
    rate_dsr <- rate_dsr %>% filter(!(d_regires_new %in% 'Galicia' & año %in% 2021))
    rate_dsr <- rate_dsr %>% filter(!(d_regires_new %in% 'Castilla y León' & año %in% 2021))
    rate_dsr <- rate_dsr %>% filter(!(d_regires_new %in% 'Cataluña' & año %in% 2021))
    rate_dsr <- rate_dsr %>% group_by(año) %>% mutate(mean_te = mean(te,na.rm=TRUE))
    
    plot <- ggplot(rate_dsr,aes(x = factor(año), y=te,tooltip = paste("Año:", año, "\nT.E.:", round(te,2),"\nMedia T.E.:", round(mean_te,2)))) +
      geom_point_interactive(size=0.5) +
      geom_line(aes(group =d_regires_new)) +
      geom_line(aes(x=factor(año), y=mean_te,color='firebrick', group =d_regires_new),linetype=2) +
      ...
      theme_minimal() + scale_color_discrete(name = "", labels = c(paste0("Media nacional (",min(rate_dsr$año,na.rm=TRUE),"-",max(rate_dsr$año,na.rm=TRUE),")"))) +
      scale_x_discrete(breaks=seq(2003, 2021, by = 3))+
...
plot_evo_temp_area <- function(ind){
    ...
    rate_dsr <- rate_dsr %>% filter(!(d_regires_new %in% 'Castilla y León' & año %in% 2021))
    rate_dsr <- rate_dsr %>% filter(!(d_regires_new %in% 'Cataluña' & año %in% 2021))
    rate_dsr1 <- rate_dsr %>% group_by(año) %>% mutate(mean_te = mean(te,na.rm=TRUE))
    ...
    aes(
      ...
      tooltip = after_stat({
        paste0(
          "\nQ1: ", round(.data$ymin,2),
          "\nQ3: ", round(.data$ymax,2),
    ...
    plot <- ggplot(data=rate_dsr2, aes(x=factor(año), y=te, group = año)) +
    geom_ribbon(aes(ymin = p05, ymax = p95,group =d_regires_new),fill = "grey70", alpha = 0.3) +
...
plot_evo_mapas_area <- function(ind){
    ...
    data <- dbGetQuery(conn = con,
                  paste0("SELECT id_episodio,
                         ...
                         gqe,
                         ",ind,"
                         FROM main.listas_quirurgicas WHERE ",ind," == true and sexo IN (1) and año IN (2016,2017,2018,2019,2020,2021)"))
    poblacion_referencia_ <- poblacion_referencia %>% filter(sexo %in% '1')
    }else{
      if(ind =='histerectomia'){
        data <- dbGetQuery(conn = con,
                  paste0("SELECT id_episodio,
                         ...
                         gqe,
                         ",ind,"
                         FROM main.listas_quirurgicas WHERE ",ind," == true and sexo IN (2) and año IN (2016,2017,2018,2019,2020,2021)"))
        poblacion_referencia_ <- poblacion_referencia %>% filter(sexo %in% '2')
      }else{
            data <- dbGetQuery(conn = con,
                  paste0("SELECT id_episodio,
                         ...
                         gqe,
                         ",ind,"
                         FROM main.listas_quirurgicas WHERE ",ind," == true and año IN (2016,2017,2018,2019,2020,2021)"))
            poblacion_referencia_ <- poblacion_referencia
      }
    }
    ...
    rate_dsr <- rate_dsr %>% filter(!(d_regires_new %in% 'Castilla y León' & año %in% 2021))
    rate_dsr <- rate_dsr %>% filter(!(d_regires_new %in% 'Cataluña' & año %in% 2021))
    mapa <- left_join(mapa_area_212,rate_dsr[c('idn212_new','año','te')],by='idn212_new')
...
sf_use_s2(FALSE)
    ...
    plot <- mapa %>%
              ...
              tm_polygons(col = "te", style = "quantile", title = "",
              palette = c('#ffffd4', '#fed98e', '#fe9929', '#d95f0e', '#993404'),group = 'año',
              legend.format = list(text.separator='-')) +
              tm_borders("gray70", lwd = 0.0) +
# tm_borders('#6e0c05', lwd = 1.2) +
              tm_layout(legend.outside = FALSE, legend.text.size = 0.8,
              main.title.size = 1, legend.show = TRUE,
...
La población utilizada en la estandarización y cálculo de los denominadores de los indicadores procede de la actualización anual de los padrones municipales publicados por el Instituto Nacional de Estadística (INE) entre los años 2003 a 2021.
