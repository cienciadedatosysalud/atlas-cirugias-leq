...
plot_evo_temp_ccaa <- function(ind){
    ...
    rate_dsr_ref <- rate_dsr_ref %>% filter(!(d_regires_new %in% 'Galicia' & año %in% 2020))
    rate_dsr_ref <- rate_dsr_ref %>% filter(!(d_regires_new %in% 'Castilla y León' & año %in% 2020))
    rate_dsr_ref <- rate_dsr_ref %>% filter(!(d_regires_new %in% 'Galicia' & año %in% 2021))
    rate_dsr_ref <- rate_dsr_ref %>% filter(!(d_regires_new %in% 'Castilla y León' & año %in% 2021))
    rate_dsr_ref <- rate_dsr_ref %>% filter(!(d_regires_new %in% 'Cataluña' & año %in% 2021))
